...
export async function GET(request: NextRequest) {
  ...
  // Only allow proxying Outline images
  if (!url.startsWith('/api/attachments.redirect') && !url.startsWith('https://docs.cdp.vn/api/attachments.redirect')) {
    return NextResponse.json({ error: 'Invalid URL' }, { status: 400 })
  }

  const outlineUrl = process.env.OUTLINE_URL || 'https://docs.cdp.vn'
  // Ensure full URL
  const targetUrl = url.startsWith('http') ? url : `${outlineUrl.replace(/\/$/, '')}${url}`
