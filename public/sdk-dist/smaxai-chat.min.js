"use strict";(()=>{async function k(){let t={userAgent:navigator.userAgent,language:navigator.language,platform:navigator.platform,screenResolution:`${screen.width}x${screen.height}`,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,colorDepth:screen.colorDepth};return"deviceMemory"in navigator&&(t.deviceMemory=navigator.deviceMemory),"hardwareConcurrency"in navigator&&(t.hardwareConcurrency=navigator.hardwareConcurrency),t}async function M(t){let a=JSON.stringify(t),i=new TextEncoder().encode(a),d=await crypto.subtle.digest("SHA-256",i);return Array.from(new Uint8Array(d)).map(m=>m.toString(16).padStart(2,"0")).join("").substring(0,32)}async function v(){let t="smaxai_user_id",a=localStorage.getItem(t);if(a)return a;let r=await k(),i=await M(r);return localStorage.setItem(t,i),i}function w(){localStorage.removeItem("smaxai_user_id")}var L={apiBaseUrl:"https://docs.cdp.vn",lang:"vi",onOpen:()=>{},onClose:()=>{}};function x(t){let a=document.createElement("div");return a.textContent=t,a.innerHTML}function h(t){return t=t.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),t=t.replace(/(?<!\*)\*([^*\n]+)\*(?!\*)/g,"<em>$1</em>"),t=t.replace(/`([^`]+)`/g,'<code class="smaxai-inline-code">$1</code>'),t=t.replace(/\[([^\]]+)\]\(([^)]+)\)/g,(a,r,i)=>{let d=i.startsWith("http")?' target="_blank" rel="noopener noreferrer"':"";return`<a class="smaxai-md-link" href="${i}"${d}>${r}</a>`}),t}function y(t){if(!t)return"";let a=t.match(/\[THINKING\]([\s\S]*?)(?:\[\/THINKING\]|$)/i),r="";if(a){let o=a[1].trim(),m=t.match(/\[\/THINKING\]/i),c="";o&&(c=o.split(`
`).map(l=>`<p class="smaxai-md-p smaxai-md-thinking-text">${h(x(l))}</p>`).join("")),r=`
...
`),d=[],e=0;for(;e<i.length;){let o=i[e];if(o.trim()===""){e++;continue}if(o.startsWith(":::")){let s=o.match(/^:::(info|warning|tip|danger|note|success)(?:\s+\[([^\]]*)\])?\s*$/);if(s){let l=s[1],n=s[2]||"",p=[];for(e++;e<i.length&&!i[e].startsWith(":::");)p.push(i[e]),e++;let g=y(p.join(`
`)),u=l==="warning"||l==="danger"?"&#9888;":l==="tip"||l==="success"?"&#10003;":"&#8505;";d.push(`<div class="smaxai-md-callout smaxai-md-callout-${l}"><span class="smaxai-md-callout-icon">${u}</span><div class="smaxai-md-callout-body">`+(n?`<div class="smaxai-md-callout-title">${x(n)}</div>`:"")+`<div class="smaxai-md-callout-content">${g}</div></div></div>`),e++;continue}}let m=o.match(/^(#{1,4})\s+(.+)$/);if(m){let s=m[1].length,l=m[2].trim();d.push(`<h${s} class="smaxai-md-h${s}">${h(l)}</h${s}>`),e++;continue}if(o.startsWith("```")){let s=o.slice(3).trim(),l=[];for(e++;e<i.length&&!i[e].startsWith("```");)l.push(i[e]),e++;let n=x(l.join(`
`));d.push('<div class="smaxai-md-code-block">'+(s?`<div class="smaxai-md-code-lang">${x(s)}</div>`:"")+`<pre class="smaxai-md-pre"><code>${n}</code></pre></div>`),e++;continue}if(o.match(/^(-{3,}|\*{3,}|_{3,})\s*$/)||o.trim()==="---"||o.trim()==="***"){d.push('<hr class="smaxai-md-hr">'),e++;continue}if(o.startsWith("> ")){let s=[];for(;e<i.length&&(i[e].startsWith("> ")||i[e].trim()==="");)i[e].startsWith("> ")?s.push(i[e].slice(2)):s.push(""),e++;d.push(`<blockquote class="smaxai-md-blockquote">${y(s.join(`
`))}</blockquote>`);continue}if(/^[-*+]\s/.test(o)){let s=[];for(;e<i.length&&/^[-*+]\s/.test(i[e]);)s.push(i[e].replace(/^[-*+]\s/,"")),e++;let l=s.map(n=>`<li>${h(x(n))}</li>`).join("");d.push(`<ul class="smaxai-md-ul">${l}</ul>`);continue}if(/^\d+\.\s/.test(o)){let s=[];for(;e<i.length&&/^\d+\.\s/.test(i[e]);)s.push(i[e].replace(/^\d+\.\s/,"")),e++;let l=s.map(n=>`<li>${h(x(n))}</li>`).join("");d.push(`<ol class="smaxai-md-ol">${l}</ol>`);continue}let c=o.match(/^!\[([^\]]*)\]\(([^)]+)\)\s*$/);if(c){d.push(`<div class="smaxai-md-img-wrap"><img class="smaxai-md-img" src="${c[2]}" alt="${x(c[1])}" loading="lazy">`+(c[1]?`<div class="smaxai-md-img-caption">${x(c[1])}</div>`:"")+"</div>"),e++;continue}if(o.includes("|")&&e+1<i.length&&i[e+1].match(/^\|?[\s\-:|]+\|?$/)){let s=[];for(;e<i.length&&i[e].includes("|");)s.push(i[e]),e++;let l=s.filter(n=>!n.match(/^\|?[\s\-:|]+\|?$/)).map(n=>n.split("|").map(p=>p.trim()).filter((p,g)=>g>0&&g<n.split("|").length-1));if(l.length>0){let n=l[0].map(g=>`<th>${h(x(g))}</th>`).join(""),p=l.slice(1).map(g=>`<tr>${g.map(u=>`<td>${h(x(u))}</td>`).join("")}</tr>`).join("");d.push(`<div class="smaxai-md-table-wrap"><table class="smaxai-md-table"><thead><tr>${n}</tr></thead><tbody>${p}</tbody></table></div>`)}continue}d.push(`<p class="smaxai-md-p">${h(x(o))}</p>`),e++}return r+d.join("")}var b=class{constructor(a={}){this.container=null;this.isOpen=!1;this.messages=[];this.userId=null;this.isLoading=!1;this.messageIdCounter=0;this.config={...L,...a},this.createUI(),this.init()}async init(){try{this.userId=await v(),await this.loadHistory(),this.messages.length>0&&this.renderMessages()}catch(a){console.error("SmaxAI Chat: Init error:",a)}}async loadHistory(){if(this.userId)try{let r=await(await fetch(`${this.config.apiBaseUrl}/api/chat/session?user_id=${this.userId}`)).json();r.success&&r.session?.messages&&(this.messages=r.session.messages)}catch(a){console.error("Failed to load chat history:",a)}}async saveHistory(){if(this.userId)try{await fetch(`${this.config.apiBaseUrl}/api/chat/session`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({user_id:this.userId,messages:this.messages})})}catch(a){console.error("Failed to save chat history:",a)}}createUI(){this.container=document.createElement("div"),this.container.id="smaxai-chat-widget",this.container.innerHTML=this.getHTML(),this.injectStyles(),document.body.appendChild(this.container),this.bindEvents()}getHTML(){return`
    ...
    `,document.head.appendChild(a)}bindEvents(){if(!this.container)return;let a=this.container.querySelector(".smaxai-trigger"),r=this.container.querySelector(".smaxai-close-btn"),i=this.container.querySelector(".smaxai-clear-btn"),d=this.container.querySelector(".smaxai-input"),e=this.container.querySelector(".smaxai-send-btn");a?.addEventListener("click",()=>this.toggle()),r?.addEventListener("click",()=>this.close()),i?.addEventListener("click",()=>this.clearChat()),d?.addEventListener("input",()=>{e?.toggleAttribute("disabled",!d.value.trim())}),d?.addEventListener("keydown",n=>{n.key==="Enter"&&!n.shiftKey&&(n.preventDefault(),this.sendMessage())}),this.container.querySelector(".smaxai-input-form")?.addEventListener("submit",n=>{n.preventDefault(),this.sendMessage()}),e?.addEventListener("click",()=>this.sendMessage());let m=this.container.querySelector(".smaxai-messages"),c=this.container.querySelector(".smaxai-image-viewer"),s=this.container.querySelector(".smaxai-image-viewer-img"),l=this.container.querySelector(".smaxai-image-viewer-close");if(m&&c&&s){m.addEventListener("click",p=>{let g=p.target;if(g.tagName.toLowerCase()==="img"&&g.classList.contains("smaxai-md-img")){let u=g;s.src=u.src,c.classList.add("open")}});let n=()=>{c.classList.remove("open"),setTimeout(()=>{c.classList.contains("open")||(s.src="")},300)};l?.addEventListener("click",n),c.addEventListener("click",p=>{p.target===c&&n()}),document.addEventListener("keydown",p=>{p.key==="Escape"&&c.classList.contains("open")&&n()})}}open(){if(!this.container)return;this.container.querySelector(".smaxai-panel")?.classList.add("open"),this.isOpen=!0,this.config.onOpen(),this.container.querySelector(".smaxai-input")?.focus(),this.renderMessages()}close(){if(!this.container)return;this.container.querySelector(".smaxai-panel")?.classList.remove("open"),this.isOpen=!1,this.config.onClose()}toggle(){this.isOpen?this.close():this.open()}async sendMessage(){if(!this.container||this.isLoading)return;let a=this.container.querySelector(".smaxai-input"),r=a?.value.trim();if(!r)return;let i={id:`msg-${++this.messageIdCounter}`,role:"user",content:r};this.messages.push(i),a.value="",this.container.querySelector(".smaxai-send-btn")?.setAttribute("disabled","true");let e={id:`msg-${++this.messageIdCounter}`,role:"assistant",content:"",isStreaming:!0};this.messages.push(e),this.renderMessages(),this.isLoading=!0;try{let o=await fetch(`${this.config.apiBaseUrl}/api/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:r,lang:this.config.lang,history:this.messages.slice(-8,-2).map(s=>({role:s.role,content:s.content}))})});if(!o.ok)throw new Error("Failed to get response");let m=o.body?.getReader();if(!m)throw new Error("No reader");let c=new TextDecoder;for(;;){let{done:s,value:l}=await m.read();if(s)break;if(l){let n=c.decode(l,{stream:!0});e.content+=n,this.renderMessages()}}e.isStreaming=!1,this.saveHistory()}catch(o){console.error("Chat error:",o),e.content="Xin l\u1ED7i, \u0111\xE3 c\xF3 l\u1ED7i x\u1EA3y ra. Vui l\xF2ng th\u1EED l\u1EA1i.",e.isStreaming=!1}finally{this.isLoading=!1,this.renderMessages()}}renderMessages(){if(!this.container)return;let a=this.container.querySelector(".smaxai-messages");if(a){if(this.messages.length===0){a.innerHTML=`
      ...
      `;return}a.innerHTML=this.messages.map(r=>`
      <div class="smaxai-message ${r.role}">
        <div class="smaxai-message-content ${r.role==="assistant"?"smaxai-md":""}">
          ${r.isStreaming&&!r.content?`
            <div class="smaxai-loading">
              <div class="smaxai-spinner"></div>
              <span>\u0110ang suy ngh\u0129...</span>
            </div>
          `:r.role==="assistant"?y(r.content):x(r.content)}
        </div>
